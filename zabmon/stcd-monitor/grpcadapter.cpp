...
Host GRPCAdapter::toHost(const proto::Host& host)
{
  Host result;

  result.name = host.name();
  result.id = host.id();
  ::google::protobuf::RepeatedPtrField<::zab::mon::proto::Interface>
...
Item GRPCAdapter::toItem(const proto::Item& item)
{
  Item result;
                             ...
                             OUT proto::ListGroupsResponse* result)
{
  for(const HostGroup& group : allGroups) {
    ::zab::mon::proto::Group* grouppt = result->add_groups();
                             ...
                             OUT proto::ListItemsValuesResponse* result)
{
  for(const ItemValues& itemValue : itemsValues) {
    ::zab::mon::proto::ItemValues* itemValuept = result->add_values();
    for(const Value& val : itemValue.values) {
      ::zab::mon::proto::Value* valuept = itemValuept->add_values();
...
HostGroup GRPCAdapter::toGroup(const proto::Group& group)
{
  HostGroup result;
...
Template GRPCAdapter::toTemplate(const proto::Template& temp)
{
  Template result;
...
void GRPCAdapter::fromHost(const Host& host, OUT proto::Host* result)
{
...
void GRPCAdapter::fromItem(const Item& item, OUT proto::Item* result)
{
